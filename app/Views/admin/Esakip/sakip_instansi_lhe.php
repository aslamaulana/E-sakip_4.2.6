			...
			<?php foreach ($sakip as $rom) : ?>
				<tr>
					...
					$j = $db->table('tb_sakip_instansi_jawaban_lhe')
						...
						->getWhere(['sakip_instansi_id' => $rom['id_sakip_instansi'], 'tb_sakip_instansi_jawaban_lhe.opd_id' => $opd_id, 'tb_sakip_instansi_jawaban_lhe.tahun' => $_SESSION['tahun']])->getRowArray();
					...
					<td>
						<a href="<?= base_url() . '/admin/esakip/sakip_instansi_lhe/sakip_instansi_history/' . $rom['id_sakip_instansi'] . '/' . $opd_id; ?>">
				...
				startRender: function(rows, group) {
					if (rows.data().pluck(2)[0] == group) {
						return $('<tr class="font-weight-bold" style="background-color: blanchedalmond;" />')
							.append('<td colspan="6" class="align-top text-wrap font-weight-bold">' + group + '</td>');
					} else if (rows.data().pluck(1)[0] == group) {
						return $('<tr class="font-weight-bold" style="background: azure;" />')
							.append('<td colspan="6" class="align-top text-wrap"><div style="padding-left: 40px;">' + group + '</div></td>');
					}
				},
				dataSrc: [2, 1]
			}
