	...
	public function index()
	{
		if (has_permission('User')) :
			$data = [
				...
				'sakip' => $this->sakip->sakip_instansi()->get()->getResultArray(),
				'db' => \Config\Database::connect(),
			];
			// dd(current_url() . ' | ' . uri_string());
			echo view('user/Esakip/sakip_instansi_lhe', $data);
		else :
	...
	public function sakip_instansi_history($id)
	{
		if (has_permission('User')) :
			$data = [
				...
				'sakip' => $this->jawaban->select('tb_sakip_instansi_jawaban_lhe.*, tb_sakip_instansi_jawaban_verifikasi_lhe.verifikasi_keterangan')->orderBy('id_sakip_instansi_jawaban', 'ASC')->join('tb_sakip_instansi_jawaban_verifikasi_lhe', 'tb_sakip_instansi_jawaban_lhe.id_sakip_instansi_jawaban = tb_sakip_instansi_jawaban_verifikasi_lhe.sakip_instansi_jawaban_id', 'LEFT')->where(['sakip_instansi_id' => $id, 'tb_sakip_instansi_jawaban_lhe.opd_id' => user()->opd_id])->findAll(),
				'komponen' => $this->sakip->sakip_instansi()->getWhere(['id_sakip_instansi' => $id])->getRowArray(),
				'db' => \Config\Database::connect(),
			];
			// dd(current_url() . ' | ' . uri_string());
			echo view('user/Esakip/sakip_instansi_history_lhe', $data);
		else :
			throw \CodeIgniter\Exceptions\PageNotFoundException::forPageNotFound();
		endif;
	}
